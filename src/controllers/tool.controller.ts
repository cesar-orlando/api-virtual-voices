...
export const createTool = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const validation = await Tool.validateToolData(req.body);
...
export const updateTool = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (Object.keys(updateData).length > 1) { // Más que solo c_name
      const validation = await Tool.validateToolData({ ...existingTool.toObject(), ...updateData });
      if (!validation.isValid) {
        res.status(400).json({ 
          message: "Tool validation failed", 
          errors: validation.errors 
...
export const testTool = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!securityValidation.isValid) {
      res.status(400).json({ 
        message: "Tool security validation failed", 
...
export const getOpenAISchema = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const schema = await Tool.generateOpenAISchemaForCompany(c_name);

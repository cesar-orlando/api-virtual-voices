...
export const getAllFacebookSessions = async (req: Request, res: Response) => {
  try {
    ...
    console.error("Error fetching Facebook sessions:", error);
    res.status(500).json({ message: "Error fetching Facebook sessions", error });
  }
};
...
export const deleteFacebookSession = async (req: Request, res: Response) => {
  ...
  if (clients[`${c_name}:${session.name}`]) {
    try {
      ...
      console.error("Error closing Facebook client:", err);
    }
  }

  // Ahora intenta borrar la carpeta
  try {
    const authDir = path.join(process.cwd(), '.fb_auth');

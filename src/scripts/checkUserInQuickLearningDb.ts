...
async function checkUserInQuickLearningDb() {
  console.log('🔍 Verificando usuario en base de datos quicklearning...\n');
  ...
  try {
    ...
    // 4. Conectar directamente a la base de datos quicklearning
    console.log('4️⃣ Conectando a base de datos quicklearning...');
    const quickLearningUri = quickLearningConfig.databaseUri;
    console.log('   - URI:', quickLearningUri);
    
    // Modificar la URI para usar la base de datos 'quicklearning'
    const uriWithDb = quickLearningUri.replace('/?', '/quicklearning?');
    ...
    // 6. Buscar usuario en la base de datos quicklearning
    console.log('6️⃣ Buscando usuario en la base de datos quicklearning...');
    const conn = await getDbConnection(currentDb);
    const User = getUserModel(conn);

    const existingUser = await User.findOne({ email: 'admin@quicklearning.com' });
    if (existingUser) {
      console.log('✅ Usuario encontrado en la base de datos quicklearning:');
      ...
      console.log('❌ Usuario no encontrado en la base de datos quicklearning');
    }

    // 7. Verificar todas las colecciones
    console.log('\n8️⃣ Verificando colecciones en la base de datos quicklearning...');
    if (!mongoose.connection.db) {
      console.log('❌ No hay conexión a la base de datos');
      return;
    }

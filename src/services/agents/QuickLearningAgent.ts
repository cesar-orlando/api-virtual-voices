...
export class QuickLearningAgent extends BaseAgent {
  ...
  protected async initializeAgent(): Promise<void> {
    ...
    this.agent = new Agent({
      ...
      console.log(`✅ QuickLearningAgent initialized for ${this.company} with ${dynamicTools.length} tools`);
            } catch (error) {
      console.error(`❌ Error initializing QuickLearningAgent for ${this.company}:`, error);
      throw error;
    }
  }
  ...
  private getSystemInstructions(): string {
    ...
    // Prompt genérico para QuickLearning
    return `Eres NatalIA, la inteligencia artificial de Quick Learning especializada en vender cursos de inglés por WhatsApp. Proporciona respuestas claras y útiles sobre nuestros cursos. Si no sabes la respuesta, di que no lo sabes y que lo vas a redirigir a un asesor. Usa las herramientas disponibles para obtener información adicional si es necesario.`;
  }
}

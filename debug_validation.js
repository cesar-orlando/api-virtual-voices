...
function getCityByLada(phone) {
  ...
  if (cleanPhone.startsWith('52') && cleanPhone.length >= 5) {
    const lada = cleanPhone.substring(2, 5);
    return LADA_TO_CITY[lada] || 'Desconocido';
  }
  return 'Desconocido';
}
...
async function debugValidation() {
  try {
    ...
    const primerAlumno = data.records[0];
    
    console.log('📋 Primer alumno original:');
    console.log(JSON.stringify(primerAlumno, null, 2));
    
    // Mapear el alumno
    const mappedAlumno = mapAlumno(primerAlumno);
    
    console.log('\n🔄 Alumno mapeado:');
    console.log(JSON.stringify(mappedAlumno, null, 2));
    
    // Crear el payload que se enviaría
    const payload = {
      tableSlug: 'alumnos',
    ...
    // Hacer la petición y mostrar el error completo
    console.log('\n🚀 Enviando petición...');

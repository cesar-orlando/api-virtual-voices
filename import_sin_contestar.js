...
function mapSinContestar(record) {
  ...
  // Mapear campos según la estructura que tengas
  // Ajusta esto según el formato de tu JSON de "sin contestar"
  for (const field of record.customFields || []) {
    switch (field.key) {
      ...
      case 'classification': fieldMap['Clasificación'] = field.value || 'prospecto'; break;
      case 'medio': fieldMap['Medio'] = field.value || 'Meta'; break;
      case 'modalidad': fieldMap['Curso'] = field.value || null; break;
      case 'campaña': fieldMap['Campaña'] = field.value || 'RMKT'; break;
      case 'comentario': fieldMap['Comentario'] = field.value || 'Sin contestar'; break;
    }
  }
  
  // Si no hay customFields, intentar mapear directamente
  if (!record.customFields) {
    fieldMap['Nombre'] = record.name || record.nombre || 'Sin nombre';
    fieldMap['Teléfono'] = record.phone || record.telefono || record.teléfono || 'Sin teléfono';
    fieldMap['Ciudad'] = getCityByLada(fieldMap['Teléfono']);
    fieldMap['Email'] = record.email || null;
    fieldMap['Clasificación'] = record.classification || record.clasificacion || 'prospecto';
    fieldMap['Medio'] = record.medio || 'Meta';
    fieldMap['Curso'] = record.curso || record.modalidad || null;
    fieldMap['Campaña'] = record.campaña || record.campana || 'RMKT';
    fieldMap['Comentario'] = record.comentario || 'Sin contestar';
  }
  ...
  if (!fieldMap['Clasificación']) fieldMap['Clasificación'] = 'prospecto';
  if (!fieldMap['Comentario']) fieldMap['Comentario'] = 'Sin contestar';
  
  return fieldMap;
}

async function importSinContestar() {
  try {
    console.log('📖 Leyendo archivo de sin contestar...');
    
    // Cambia el nombre del archivo según tengas
    const raw = fs.readFileSync('sin_contestar.json', 'utf8');
    const data = JSON.parse(raw);
    const records = data.records || data; // Soporta ambos formatos
    
    console.log(`📊 Total de registros a importar: ${records.length}`);
    
    let successCount = 0;
    let errorCount = 0;
    let linkedToChatCount = 0;
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      console.log(`   📱 Teléfono: ${mappedRecord.Teléfono}`);
      console.log(`   🏙️  Ciudad: ${mappedRecord.Ciudad}`);
      
      try {
        // Crear el registro
        const response = await axios.post('http://localhost:3001/api/records', {
          tableSlug: 'sin_contestar', // Cambia según tu tabla
          c_name: 'quicklearning',
          createdBy: 'admin@quicklearning.com',
          data: {
            Nombre: mappedRecord.Nombre,
            Teléfono: mappedRecord.Teléfono,
            Email: mappedRecord.Email,
            Clasificación: mappedRecord.Clasificación,
            Medio: mappedRecord.Medio,
            Curso: mappedRecord.Curso,
            Ciudad: mappedRecord.Ciudad,
            Campaña: mappedRecord.Campaña,
            Comentario: mappedRecord.Comentario
          }
        });
        ...
        // Intentar vincular con chat existente
        try {
          const phone = mappedRecord.Teléfono.replace(/\D/g, '');
          if (phone && phone !== 'Sinteléfono') {
            // Buscar chat por teléfono
            const chatResponse = await axios.get(`http://localhost:3001/api/whatsapp/chat/${phone}`);
            if (chatResponse.data && chatResponse.data.chat) {
              console.log(`   🔗 Vinculado con chat existente`);
              linkedToChatCount++;
            }
          }
        } catch (chatError) {
          console.log(`   ⚠️  No se encontró chat para este número`);
        }
    ...
    console.log(`🔗 Vinculados con chat: ${linkedToChatCount}`);
...
// Función para crear un ejemplo de JSON si no lo tienes
    ...
    records: [
      {
        ...
        comentario: "Sin contestar - primer contacto"
      },
      {
        name: "María García",
        phone: "5215523456789",
        ...
        classification: "prospecto",
        modalidad: "Virtual",
        comentario: "Sin contestar - segundo intento"
      }
    ]
  };
  
  fs.writeFileSync('sin_contestar_example.json', JSON.stringify(exampleData, null, 2));
  console.log('📝 Archivo de ejemplo creado: sin_contestar_example.json');
}

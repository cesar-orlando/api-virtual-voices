 ...
 * 
 * Uso: node scripts/quick-advisor-report.js
 * 
 * IMPORTANTE: Asegúrate de tener configurada la variable MONGO_URI en tu .env
 * o pásala como parámetro: MONGO_URI="tu_uri" node scripts/quick-advisor-report.js
 */

const mongoose = require('mongoose');
require('dotenv').config();

// Configuración de conexión - usar la variable específica de QuickLearning
...
async function generateQuickReport() {
  try {
    ...
    if (advisorsWithResponses.length === 0) {
      ...
      console.log("\n💡 Posibles causas:");
      console.log("- No hay chats con asesores asignados");
      console.log("- Los mensajes no tienen respondedBy: 'asesor'");
      console.log("- Los asesores no han respondido mensajes");
      return;
    }
    ...
    console.log("\n💡 Puedes hacer screenshot de este reporte para enviar al cliente");
    
  } catch (error) {
    console.error("❌ Error generando reporte:", error);
    console.log("\n🔧 Soluciones posibles:");
    console.log("1. Verifica que la base de datos esté corriendo");
    console.log("2. Configura MONGO_URI en tu archivo .env");
    console.log("3. Usa: MONGO_URI='tu_uri_mongodb' node scripts/quick-advisor-report.js");
  } finally {
    // Cerrar conexión
